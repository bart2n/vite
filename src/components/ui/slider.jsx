...
function SliderThumb({ className, ...props }) {
  return (
    <SliderPrimitive.Thumb
      ...
      className={cn(
        'box-content block size-4 shrink-0 cursor-pointer rounded-full border-[2px] border-primary bg-primary-foreground shadow-sm shadow-black/5 outline-hidden focus:outline-hidden',
        className,
      )}
      {...props}
    />
